...
11. [보너스 기능 4가지 상세](#11-보너스-기능-4가지-상세)
    - [11-1. 멀티 프로바이더 / 모델 비교 / 측면 만족도](#11-1-추가-기여-멀티-프로바이더--모델-비교--측면-만족도)
...
기록됩니다. `config.json`의 `ai.provider`/모델을 바꿔서(예: Claude → Spark 로컬
Qwen) `analyze --all`을 다시 돌리면 새 스냅샷이 하나 더 쌓이고, 그 두 스냅샷의
...
## 11-1.  멀티 프로바이더 / 모델 비교 / 측면 만족도

[#11-1-멀티-프로바이더--모델-비교--측면-만족도](#11-1-팀원-추가-기여-멀티-프로바이더--모델-비교--측면-만족도)
